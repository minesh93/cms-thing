        ...
        <form class="row" v-on:submit="savePost">
            <div class="col-xs-9">
                <div class="row">
                    <div class="col-xs-12">
                        <div class="fieldset">
                            <input type="text" class="large-field" placeholder="Username" v-model="user.name">
                        </div>
                    </div>
                    ...
                    <div class="col-xs-12">
                        <div class="fieldset">
                            <input type="password" class="large-field" placeholder="Password" v-model="user.password">
                        </div>
                    </div>

                    <div class="col-xs-12">
                        <div class="fieldset">
                            <multiselect class="large-field" v-model="user.role"  track-by="name" label="name" placeholder="Role" :options="roles" :searchable="false" :show-labels="false"></multiselect>
                        </div>
                    </div>
                </div>
...
<script>
    export default {
        ...
        methods:{
            savePost(e){
                e.preventDefault();
                let location = `/admin/content/${this.user.type}/add`;
                let newPost = true;
                if(this.user.id != undefined){
                    newPost = false;
                    location = `/admin/content/${this.user.type}/${this.user.id}`;
                }
                axios.post(location,this.user).then((response)=>{
                    this.$parent.$emit('make-notification',{text:'Post Saved!',type:'is-success'});
                    this.user = response.data;
                    if(newPost){
                       window.location = `/admin/content/${this.user.type}/${this.user.id}`; 
                    }
                }).catch((error)=>{
